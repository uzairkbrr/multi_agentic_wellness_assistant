...
mode = st.radio("Mode", ["View", "Edit"], index=0, horizontal=True)
...
if mode == "Edit":
    with st.form("profile_form"):
        ...
        dietary_preferences = st.text_input("Dietary Preferences (e.g., vegan, keto, allergies)", value=u.get("dietary_preferences") or "")
        mental_health_background = st.text_area("Mental Health Background (optional)", value=u.get("mental_health_background") or "")
        # time availability as dropdowns
        time_options = [f"{h:02d}:00" for h in range(0,24)]
        colt1, colt2 = st.columns(2)
        with colt1:
            start_time = st.selectbox("Day Start Time", time_options, index=8)
        with colt2:
            end_time = st.selectbox("Day End Time", time_options, index=20)
        daily_schedule = f"{start_time}-{end_time}"
        medical_conditions = st.text_area("Medical conditions (if any)", value=u.get("medical_conditions") or "")
...
with colA:
    ...
    if upload is not None:
        try:
            ...
            filename = f"user_{u['id']}_{int(st.session_state.get('ts', 0) or 0)}_{upload.name}"
...
if mode == "View":
    ...
    with col2:
        ...
        st.markdown(f"**Schedule**: {u.get('daily_schedule','-')}")
